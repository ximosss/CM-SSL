...
from baselines.BYOL.byol_pretraining import BYOL 
...
def main(cfg: DictConfig):
    ...
    byol_model_to_load = BYOL(
        net=byol_backbone,
        hidden_layer=-1,  
        projection_size=cfg.model.projection_dim,
    ...
    if rank == 0:
        log.info(f"Loading pretrained BYOL weights from: {cfg.model.pretrained_model_path}")

    pretrained_dict = torch.load(cfg.model.pretrained_model_path, map_location='cpu')
    byol_model_to_load.load_state_dict(pretrained_dict, strict=False)
    if rank == 0:
        log.info("BYOL weights loaded successfully.")

    backbone = byol_model_to_load.online_encoder.net
    feature_dim = backbone.final_bn.num_features
