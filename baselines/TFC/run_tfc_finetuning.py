...
from baselines.TFC.tfc_pretraining import TFC_Model
...
def main(cfg: DictConfig):
    ...
    if rank == 0:
        print(f"Loading pretrained TFC weights from: {cfg.model.pretrained_model_path}")

    pretrained_dict = torch.load(cfg.model.pretrained_model_path, map_location='cpu')
    tfc_model.load_state_dict(pretrained_dict)

    if rank == 0:
        print("TFC weights loaded successfully.")

    backbone = tfc_model.time_encoder
    feature_dim = cfg.model.projection_dim
