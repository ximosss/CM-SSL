...
from baselines.PaPaGei.resnet import ResNet1DMoE
from baselines.PaPaGei.utils import load_model_without_module_prefix
...
class FineTuningModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # PaPaGei returns a tuple (embeddings, expert_outputs, gating_weights)g
...
def main(cfg: DictConfig):
    ...
    if rank == 0:
        log.info("Initializing PaPaGei model from config...")
    
    backbone = ResNet1DMoE(
    ...
    if rank == 0: 
        log.info(f"Loading pretrained PaPaGei weights from: {cfg.model.pretrained_model_path}")
    backbone = load_model_without_module_prefix(backbone, cfg.model.pretrained_model_path)
    
    feature_dim = cfg.model.n_classes  
